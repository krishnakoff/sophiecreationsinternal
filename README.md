...
- **`leads`** — one row per company, moving through a `stage`: `prospect` -> `contacted` ->
  ...
  `contacted` to `conversation`. A signed `client` who's gone quiet but isn't dead can move to
  `revive` — still worth actively pushing for another order — and from there back to `client`
  (they ordered again) or on to `dead`. `contacted_at` + `steps_completed` (0–4) drive the day
  1 / 4 / 7 / 10 auto follow-up cadence shown in the "Next action" column while a lead is
  `contacted`; past that, `next_action_date`/`next_action_type` are the manual "what's next"
  fields. `prospect` -> `contacted` and `contacted` -> `conversation` are flipped automatically
  by Claude scanning Gmail (see `CLAUDE.md`); `sampling`/`client`/`revive`/`dead` are manual
  calls. Every stage card in the CRM is clickable — it filters the table to just that stage,
  scoped to whichever tab (Krishna's or Sanjay's) is currently open. A `priority` flag pins a
  lead to the top of the table (with a star) regardless of
